...
public sealed class ProjectCommand : InteractionModuleBase
{
								   ...
								   Autocomplete(typeof(ProjectTypeAutoCompleteHandler))] string template)
	{
		...
		var role = await ServerService.GetServer(Context.Guild.Id).CreateRoleAsync($"{name} - Project", isMentionable: true);
		var category = await ServerService.GetServer(Context.Guild.Id).CreateCategoryChannelAsync(name);
